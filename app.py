...
model_base = YOLOv8ObjectDetector(model_path=model_base_path, device=DEVICE, img_size=IMG_SIZE, confidence=0.3)
cam_base = YOLOV8GradCAM(model=model_base, layer_name=TARGET_LAYER, img_size=IMG_SIZE)

model_jerome = YOLOv8ObjectDetector(model_path=model_jerome_path, device=DEVICE, img_size=IMG_SIZE, confidence=0.3)
...
def process_in_memory(model, cam_wrapper, img_bgr, forced_label=None):
    h, w = img_bgr.shape[:2]
    original_shape = (h, w)
    ...
    if valid_masks:
        ...
        for bbox, label in zip(valid_boxes, valid_labels):
            ...
            cv2.putText(res_img, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    return image_to_base64(res_img), len(valid_boxes)
...
def compare():
    try:
        ...
        start = time.time()
        res_base_b64, count_base = process_in_memory(model_base, cam_base, img_bgr, forced_label=DISPLAY_LABEL)
        time_base = time.time() - start

        start = time.time()
        res_jerome_b64, count_jerome = process_in_memory(model_jerome, cam_jerome, img_bgr, forced_label=DISPLAY_LABEL)
        time_jerome = time.time() - start

        return jsonify({
            'result_jerome': res_base_b64,
            'result_yolov8n': res_jerome_b64,
